...
function usage() {
  ...
  * an Oracle database (PDB) instance in the cluster
  ...
  OPTIONAL FLAGS
     ...
     -k, --cluster_name
        Name of the GKE cluster to be created, default value is 'gkecluster'
     -z, --gke_zone
...
function parse_options() {
  ...
  if [[ -z ${GCS_PATH+x} || -z ${GKE_SA+x} ]]; then
    usage
  fi

  [[ $GCS_PATH = gs://* ]] || (echo "{-g,--gcs_oracle_binaries_path} should start with gs://"; exit 1)
  [[ $GCS_PATH = gs://gs://* ]] && (echo "{-g,--gcs_oracle_binaries_path} should have format gs://bucket[/path], gs://gs:// is an invalid prefix"; exit 1)
  [[ "$GCS_PATH" =~ ^gs://[a-z0-9][a-z0-9._-]+[a-z0-9](/.*)?$ ]] || (echo "{-g,--gcs_oracle_binaries_path} bucket name is invalid, see https://cloud.google.com/storage/docs/naming-buckets"; exit 1)
}
