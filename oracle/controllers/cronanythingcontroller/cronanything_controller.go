...
// +kubebuilder:rbac:groups=oracle.db.anthosapis.com,resources=cronanythings,verbs=*
...
func (r *ReconcileCronAnything) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	canonicalName := fmt.Sprintf("%s/%s", instance.Namespace, instance.Name)
	now := r.currentTime()
	...
	if len(droppedSchedules) > 0 {
		...
		if len(droppedSchedules) == 1 && instance.Status.PendingTrigger != nil && instance.Status.PendingTrigger.ScheduleTime.Equal(getMetaTimePointer(latestDropped)) {
			...
			historyRecord.Result = instance.Status.PendingTrigger.Result
		} else {
			historyRecord.Result = cronanything.TriggerResultMissed
		}

		err := r.updateCronAnythingStatus(instance.Name, instance.Namespace, func(freshStatus *cronanything.CronAnythingStatus) {
	...
	if instance.Spec.TriggerDeadlineSeconds != nil {
		triggerDeadline := time.Duration(*instance.Spec.TriggerDeadlineSeconds)
	...
	if instance.Spec.TotalResourceLimit != nil && int32(len(childResources)) >= *instance.Spec.TotalResourceLimit {
		log.Info("Resource limit info", "caName", canonicalName, "limit", *instance.Spec.TotalResourceLimit, "numChildResources", len(childResources))
	...
	labels[cronanything.CronAnythingCreatedByLabel] = instance.Name
	labels[cronanything.CronAnythingScheduleTimeLabel] = strconv.FormatInt(scheduleTime.Unix(), 10)
	crt.SetLabels(labels)
	if instance.Spec.CascadeDelete != nil && *instance.Spec.CascadeDelete {
		crt.SetOwnerReferences([]metav1.OwnerReference{*metav1.NewControllerRef(instance, controllerKind)})
	}

	err = r.resourceControl.Create(cgvr, instance.Namespace, crt)
	if err != nil {
		statusErr := r.updateCronAnythingStatus(instance.Name, instance.Namespace, func(freshStatus *cronanything.CronAnythingStatus) {
...
func (r *ReconcileCronAnything) cleanupHistory(ca *cronanything.CronAnything, childResources []*unstructured.Unstructured, resource schema.GroupVersionResource, now time.Time) []*unstructured.Unstructured {
	if ca.Spec.Retention == nil {
		return childResources
	}
	historyTimeLimit := ca.Spec.Retention.HistoryTimeLimitSeconds
	historyCountLimit := ca.Spec.Retention.HistoryCountLimit
...
func isFinished(ca *cronanything.CronAnything, resource *unstructured.Unstructured) (bool, error) {
	if ca.Spec.FinishableStrategy == nil {
		return false, nil
	}
	switch ca.Spec.FinishableStrategy.Type {
	case cronanything.FinishableStrategyTimestampField:
		strategy := ca.Spec.FinishableStrategy.TimestampField
		if strategy == nil {
	...
	case cronanything.FinishableStrategyStringField:
		strategy := ca.Spec.FinishableStrategy.StringField
		if strategy == nil {
...
func getResourceTimestamp(ca *cronanything.CronAnything, resource *unstructured.Unstructured) (time.Time, error) {
	switch ca.Spec.Retention.ResourceTimestampStrategy.Type {
	case cronanything.ResourceTimestampStrategyField:
		strategy := ca.Spec.Retention.ResourceTimestampStrategy.FieldResourceTimestampStrategy
...
func (r *ReconcileCronAnything) getChildResources(ca *cronanything.CronAnything, template *unstructured.Unstructured, resource schema.GroupVersionResource) ([]*unstructured.Unstructured, error) {
	return r.resourceControl.List(resource, ca.Name)
}
...
func getResourceName(ca *cronanything.CronAnything, scheduleTime time.Time) string {
	baseName := ca.Name
	if ca.Spec.ResourceBaseName != nil && len(*ca.Spec.ResourceBaseName) > 0 {
		baseName = *ca.Spec.ResourceBaseName
	}

	timestamp := fmt.Sprintf("%d", scheduleTime.Unix())
	if ca.Spec.ResourceTimestampFormat != nil && len(*ca.Spec.ResourceTimestampFormat) > 0 {
		timestamp = scheduleTime.Format(*ca.Spec.ResourceTimestampFormat)
	}
...
func templateToUnstructured(ca *cronanything.CronAnything) (*unstructured.Unstructured, error) {
	...
	err := json.Unmarshal(ca.Spec.Template.Raw, &template)
	if err != nil {
